...
- [x] Schedule routines by weekday (workout_templates.scheduled_days in 202609080009_scheduled_days.sql, day checkboxes in the routine editor, a training-week overview, and today's routine first in Log), link every exercise name to its stats page, and add Week/Month/Year ranges to the exercise chart with weekly bucketing for long spans.
- [x] Give every screen a shared five-tab navigation (Log, Routines, Exercises, Calendar, Progress), fix phone viewport overflow (scrollable chart ranges, clipped chart overflow, wrapping headings and routine actions), remove the unused entry-mode setting, and grow the standard catalog from 150 to 162 in 202609080010_exercises_extra.sql.
- [ ] Apply 202609080005_rep_ranges.sql, 202609080006_catalog.sql, 202609080007_plates.sql, 202609080008_guards.sql, 202609080009_scheduled_days.sql and 202609080010_exercises_extra.sql to the development and production Supabase projects; see the pending migrations section in SETUP.md.
